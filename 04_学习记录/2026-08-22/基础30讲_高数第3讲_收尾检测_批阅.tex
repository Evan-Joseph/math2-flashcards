...
\textcolor{problemred}{$\displaystyle\lim_{x\to0}\frac{f(\sin x)}x=\lim_{x\to0}f'(\sin x)\cdot\cos x$}。\quad\problem{题设只给出 $f$ 在 $0$ 处可导，不能保证 $f$ 在 $0$ 的去心邻域内可导，洛必达法的条件不足。}\\
\textcolor{problemred}{$=f'(\cos x)$}\quad\problem{即使另有条件保证可以使用洛必达，这里也应继续取 $x\to0$ 的极限，结论应为 $f'(0)$。}
\pt{正确解答}
