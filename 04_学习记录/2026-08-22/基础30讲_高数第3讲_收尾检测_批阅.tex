...
$\displaystyle\lim_{x\to0}\frac{f(\sin x)}x=\lim_{x\to0}\frac{f'(\sin x)\cdot\cos x}{x}$。\quad\problem{你确实写了这一整行；问题在于把原函数值比值直接换成了导数表达式。洛必达若要使用，必须先确认分子在邻域内可导，并且求导后分母为 $1$。}\\
$=f'(\cos x)$
\pt{正确解答}
...
\pt{正确解答}
\fix{保留 $a=b$；补足极限：}$当 $b=a$ 时，$\displaystyle f'(0)=\lim_{x\to0}\frac{\sin(ax)-ax}{x^2}=0$。所以 $\boxed{a=b}$，$a$ 任意。
