...
import { CARD_KID, EXT_STATUS, KNOWLEDGE_GROUPS } from './knowledge';

export { KNOWLEDGE_GROUPS } from './knowledge';
...
export const ALL_CARDS: KCard[] = BASE.map((c) => {
  ...
  return {
    ...
    status: c.status ?? (EXT_STATUS[c.id] ? 'ext' : 'ok'),
    src: c.src ?? ch.secSrc?.[c.sec] ?? ch.src,
    kid: c.kid ?? CARD_KID[c.id] ?? c.id,
  };
...
/** 是否进入默认学习 / 复习队列：已核验（非待确认、非拓展） */
export function isStudyable(card: KCard): boolean {
  return (card.status ?? 'ok') === 'ok';
}

/** 进入默认学习/复习队列的卡片：范围内且已核验 */
export function studyableFor(exam: Exam): KCard[] {
  return ALL_CARDS.filter((c) => cardInExam(c, exam) && isStudyable(c));
}

/** 知识点 id → 该知识点的全部卡片 */
export const KID_CARDS: Record<string, KCard[]> = (() => {
  const m: Record<string, KCard[]> = {};
  for (const c of ALL_CARDS) (m[c.kid!] ??= []).push(c);
  return m;
})();

/** 唯一知识点数（按 kid 去重） */
export function countKnowledge(cards: KCard[]): number {
  return new Set(cards.map((c) => c.kid!)).size;
}

/** 同一知识点的其他卡片 */
export function siblingsOf(card: KCard, exam: Exam): KCard[] {
  return (KID_CARDS[card.kid!] ?? []).filter((c) => c.id !== card.id && cardInExam(c, exam));
}
