...
**要点**：$f_1'$、$f_2'$ 仍是 $u,v$ 的复合函数，再求导时要**继续用链式法则**：$\dfrac{\partial f_1'}{\partial y}=f_{11}''u_y+f_{12}''v_y$。`,
  ...
  },
  {
    ...
    cond: r`必要条件要求 $f$ 在该点两个偏导都存在；充分条件要求 $f$ 在驻点某邻域内具有**二阶连续偏导数**。`,
...
- $AC-B^2=0$：无法判定，需用定义（沿特殊路径比较）。`,
  ...
  },
  {
    ...
    cond: r`$f,\varphi$ 具有一阶连续偏导数，且在所考察的点处 $\nabla\varphi=(\varphi_x,\varphi_y,\varphi_z)\ne\mathbf 0$（约束正则）。若 $\nabla\varphi=\mathbf 0$ 的点落在约束曲面上，须单独作为可疑点检验。`,
...
3. 比较所有候选值，最大者为最大值，最小者为最小值。`,
  ...
  },
  {
    ...
    q: r`用变量代换化简含二阶偏导的方程（如令 $u=x+ay,v=x+by$，把关于 $z(x,y)$ 的方程化为关于 $u,v$ 的方程）的步骤`,
    cond: r`要求 $z$ 具有二阶连续偏导数（保证 $z_{uv}=z_{vu}$）。这是多元复合函数求导的计算题（数学一、二真题均出现过），不涉及偏微分方程理论。`,
...
**齐次**：$\dfrac{dy}{dx}=\varphi\Big(\dfrac yx\Big)$，令 $u=\dfrac yx$，$y'=u+xu'$，化为 $xu'=\varphi(u)-u$（可分离）。`,
  ...
  {
    sec: '一阶方程', k: 'cloze', s: 2, scope: 'm1',
    q: r`伯努利方程 $y'+P(x)y=Q(x)y^n$：两边除以 $y^n$，令 $z=⟦y^{1-n}⟧$，得线性方程 $z'+⟦(1-n)P(x)⟧z=(1-n)Q(x)$`,
    cond: r`$n\ne0,1$（$n=0$ 为一阶线性，$n=1$ 为可分离变量）；除以 $y^n$ 时默认 $y\ne0$，$n>0$ 时 $y\equiv0$ 也是解。`,
    hook: '数学一大纲单列伯努利方程；数学二大纲不含，仅在数学一模式出现。',
  },
  {
    sec: '可降阶方程', k: 'qa', s: 3,
    q: '三类可降阶高阶方程的降阶方法',
